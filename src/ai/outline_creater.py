...
class CreateOutline(dspy.Signature):
    """あなたは、英語教育の専門家です。最新の英語教育研究や学習法を常にフォローし、理解しやすい解説を提供できるよう心がけています。
...
4. **タイトルの形式**：
   - "# タイトル" をレポートのタイトルに用いる（必ず日本語、読者を惹きつける表現）
   - "## セクション名" を章のタイトルとして用いる（必ず日本語）
   - "### サブセクション名" を節のタイトルとして用いる（必ず日本語）
   
   **タイトルの魅力向上**：
   - 「完全ガイド」「徹底解説」「マスター法」などの魅力的な表現を使用
   - 「〜を理解する」「〜を習得する」「〜を克服する」などの行動指向的な表現
   - 「実践的」「効果的」「体系的」などの価値を示す形容詞を活用
   - 学習者の興味を引く具体的なメリットをタイトルに含める
...
class FixOutline(dspy.Signature):
    """あなたは、英語学習教材の編集者です。英語学習の解説レポートのアウトラインを作成しましたが、下記のルールに従っていない部分があるかもしれません。校正・編集して、理解しやすい構成にしてください。
...
4. **学習者目線の構成**：
   - クエリーに対して、理解しやすい英語教育的な観点からの解説・分析を目的とした構成にする
